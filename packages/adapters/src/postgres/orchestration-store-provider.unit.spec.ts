...
describe("PostgresOrchestrationStoreProvider", () => {
  ...
  it("executes rollback files in reverse order", async () => {
    ...
    expect(statements[1]).toContain('DELETE FROM "runs" WHERE "parent_kind" = \'project\'');
    expect(statements[2]).toContain("DROP TABLE IF EXISTS whatsapp_accounts");
    expect(statements[3]).toContain('DROP TABLE IF EXISTS "artifacts"');
    expect(statements[4]).toContain('DROP TABLE IF EXISTS "deployments"');
    expect(statements[4]).toContain('DROP TABLE IF EXISTS "projects"');
    expect(statements[5]).toContain('DROP CONSTRAINT "node_executions_status_check"');
    expect(statements[6]).toContain('DROP INDEX IF EXISTS "idx_recovery_actions_pending_node"');
    expect(statements[7]).toContain('DROP TABLE IF EXISTS "approvals"');
    expect(statements[8]).toContain('DROP TABLE IF EXISTS "run_outcomes"');
    expect(statements[9]).toContain('DROP TABLE IF EXISTS "recovery_actions"');
    expect(statements[10]).toContain('DROP TABLE IF EXISTS "verification_results"');
    expect(statements[11]).toContain('DROP CONSTRAINT IF EXISTS "runs_workflow_version_tenant_fk"');
    expect(statements[11]).toContain('DROP COLUMN IF EXISTS "workflow_version_id"');
    expect(statements[12]).toContain('DROP TABLE IF EXISTS "run_stream_events"');
    expect(statements[13]).toContain('DROP TABLE IF EXISTS "node_executions"');
    expect(statements[14]).toContain('DROP TABLE IF EXISTS "blackboard_checkpoints"');
    expect(statements[15]).toContain("workflow_versions_canary_traffic_check");
    expect(statements[16]).toContain("workflow_versions_reject_tenant_id_change");
    expect(statements[17]).toContain("conversation_goal_states_reject_tenant_id_change");
    expect(statements[18]).toContain("runs_reject_tenant_id_change");
    expect(statements[23]).toContain("workflows_reject_tenant_id_change");
    expect(statements[24]).toBe("DROP SCHEMA IF EXISTS drizzle CASCADE");
    expect(statements[25]).toBe("COMMIT");
    expect(client.release).toHaveBeenCalledOnce();
  });
