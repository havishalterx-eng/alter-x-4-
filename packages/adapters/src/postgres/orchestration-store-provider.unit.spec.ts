...
describe("PostgresOrchestrationStoreProvider", () => {
  ...
  it("executes rollback files in reverse order", async () => {
    ...
    expect(statements[1]).toContain('DROP TABLE IF EXISTS "artifacts"');
    expect(statements[2]).toContain('DROP TABLE IF EXISTS "deployments"');
    expect(statements[2]).toContain('DROP TABLE IF EXISTS "projects"');
    expect(statements[3]).toContain('DROP CONSTRAINT "node_executions_status_check"');
    expect(statements[4]).toContain('DROP INDEX IF EXISTS "idx_recovery_actions_pending_node"');
    expect(statements[5]).toContain('DROP TABLE IF EXISTS "approvals"');
    expect(statements[6]).toContain('DROP TABLE IF EXISTS "run_outcomes"');
    expect(statements[7]).toContain('DROP TABLE IF EXISTS "recovery_actions"');
    expect(statements[8]).toContain('DROP TABLE IF EXISTS "verification_results"');
    expect(statements[9]).toContain('DROP CONSTRAINT IF EXISTS "runs_workflow_version_tenant_fk"');
    expect(statements[9]).toContain('DROP COLUMN IF EXISTS "workflow_version_id"');
    expect(statements[10]).toContain('DROP TABLE IF EXISTS "run_stream_events"');
    expect(statements[11]).toContain('DROP TABLE IF EXISTS "node_executions"');
    expect(statements[12]).toContain('DROP TABLE IF EXISTS "blackboard_checkpoints"');
    expect(statements[13]).toContain("workflow_versions_canary_traffic_check");
    expect(statements[14]).toContain("workflow_versions_reject_tenant_id_change");
    expect(statements[15]).toContain("conversation_goal_states_reject_tenant_id_change");
    expect(statements[16]).toContain("runs_reject_tenant_id_change");
    expect(statements[21]).toContain("workflows_reject_tenant_id_change");
    expect(statements[22]).toBe("DROP SCHEMA IF EXISTS drizzle CASCADE");
    expect(statements[23]).toBe("COMMIT");
    expect(client.release).toHaveBeenCalledOnce();
  });
