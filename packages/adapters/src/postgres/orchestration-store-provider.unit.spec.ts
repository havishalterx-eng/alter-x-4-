...
describe("PostgresOrchestrationStoreProvider", () => {
  ...
  it("executes rollback files in reverse order", async () => {
    ...
    expect(statements[1]).toContain('DROP TABLE IF EXISTS "run_stream_events"');
    expect(statements[2]).toContain('DROP TABLE IF EXISTS "node_executions"');
    expect(statements[3]).toContain('DROP TABLE IF EXISTS "blackboard_checkpoints"');
    expect(statements[4]).toContain("workflow_versions_canary_traffic_check");
    expect(statements[5]).toContain("workflow_versions_reject_tenant_id_change");
    expect(statements[6]).toContain("conversation_goal_states_reject_tenant_id_change");
    expect(statements[7]).toContain("runs_reject_tenant_id_change");
    expect(statements[12]).toContain("workflows_reject_tenant_id_change");
    expect(statements[13]).toBe("DROP SCHEMA IF EXISTS drizzle CASCADE");
    expect(statements[14]).toBe("COMMIT");
    expect(client.release).toHaveBeenCalledOnce();
  });
