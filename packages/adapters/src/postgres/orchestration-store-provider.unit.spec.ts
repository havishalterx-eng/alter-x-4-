...
describe("PostgresOrchestrationStoreProvider", () => {
  ...
  it("executes rollback files in reverse order", async () => {
    ...
    expect(statements[1]).toContain('DROP INDEX IF EXISTS "idx_deployments_tenant_project_active"');
    expect(statements[2]).toContain('DELETE FROM "runs" WHERE "parent_kind" = \'project\'');
    expect(statements[3]).toContain("DROP TABLE IF EXISTS whatsapp_accounts");
    expect(statements[4]).toContain('DROP TABLE IF EXISTS "artifacts"');
    expect(statements[5]).toContain('DROP TABLE IF EXISTS "deployments"');
    expect(statements[5]).toContain('DROP TABLE IF EXISTS "projects"');
    expect(statements[6]).toContain('DROP CONSTRAINT "node_executions_status_check"');
    expect(statements[7]).toContain('DROP INDEX IF EXISTS "idx_recovery_actions_pending_node"');
    expect(statements[8]).toContain('DROP TABLE IF EXISTS "approvals"');
    expect(statements[9]).toContain('DROP TABLE IF EXISTS "run_outcomes"');
    expect(statements[10]).toContain('DROP TABLE IF EXISTS "recovery_actions"');
    expect(statements[11]).toContain('DROP TABLE IF EXISTS "verification_results"');
    expect(statements[12]).toContain('DROP CONSTRAINT IF EXISTS "runs_workflow_version_tenant_fk"');
    expect(statements[12]).toContain('DROP COLUMN IF EXISTS "workflow_version_id"');
    expect(statements[13]).toContain('DROP TABLE IF EXISTS "run_stream_events"');
    expect(statements[14]).toContain('DROP TABLE IF EXISTS "node_executions"');
    expect(statements[15]).toContain('DROP TABLE IF EXISTS "blackboard_checkpoints"');
    expect(statements[16]).toContain("workflow_versions_canary_traffic_check");
    expect(statements[17]).toContain("workflow_versions_reject_tenant_id_change");
    expect(statements[18]).toContain("conversation_goal_states_reject_tenant_id_change");
    expect(statements[19]).toContain("runs_reject_tenant_id_change");
    expect(statements[24]).toContain("workflows_reject_tenant_id_change");
    expect(statements[25]).toBe("DROP SCHEMA IF EXISTS drizzle CASCADE");
    expect(statements[26]).toBe("COMMIT");
    expect(client.release).toHaveBeenCalledOnce();
  });
