...
async def test_grpc_interceptor_aborts_without_a_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("INTERNAL_SERVICE_TOKEN_SHA256", _sha256(TOKEN))
    interceptor = ServiceAuthInterceptor()
    sentinel = object()

    async def continuation(details: _t.Any) -> _t.Any:
        return sentinel

    handler = await interceptor.intercept_service(
        continuation, _CallDetails("/alter.capability.v1.CapabilityService/Resolve", ())
    )
    # Without a token the interceptor must NOT hand control to the real handler.
    assert handler is not sentinel


async def test_grpc_interceptor_passes_through_with_a_valid_token(
    ...
    handler = await interceptor.intercept_service(
        continuation,
        _CallDetails(
            "/alter.capability.v1.CapabilityService/Resolve",
...
def test_protected_route_rejects_a_wrong_token() -> None:
    response = _client().get(
        "/planner/anything", headers={"authorization": "Bearer wrong"}
    )
    assert response.status_code == 401
...
def test_protected_route_accepts_a_valid_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    print("DETAIL:", response.status_code, response.text)
