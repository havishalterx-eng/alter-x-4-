...
class ParagraphAwareChunkSplitter:
    ...
    the whole document becomes exactly one opaque chunk. Real per-page/
    per-region chunking needs a PDF-text-extraction/OCR ticket first.
    """
    ...
    def split(self, *, content: bytes, content_type: str) -> tuple[Chunk, ...]:
        if content_type not in _TEXT_LIKE_CONTENT_TYPES:
            ...
            # is not real chunk content anyway. This placeholder describes
            # what the chunk *is* (real, honest metadata), not what it
            # contains (which no extractor here can read).
            placeholder = (
                f"[{content_type} content, {len(content)} bytes -- "
                "no text extraction available]"
            )
            return (Chunk(seq=0, text=placeholder),)
